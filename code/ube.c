...
void ube(double *X, int *no_point, int no_dim_in, int no_dim, int knn, int min_knn, int kernel_poly_degree, int is_newton, int is_euclidean, int no_thread, double *Y)
{
	...
	if (no_thread > 1)
	{
		dist_to_affinity_elbow_omp(affinity, (*no_point), knn, min_knn, no_thread);
		ube_optimize_omp(nn_idx, affinity, (*no_point), no_dim, knn, kernel_poly_degree, is_newton, no_thread, Y);
	}
	else
	{
		dist_to_affinity_elbow(affinity, (*no_point), knn, min_knn);
		ube_optimize(nn_idx, affinity, (*no_point), no_dim, knn, kernel_poly_degree, is_newton, Y);
	}
...
void ube_optimize(int *nn_idx, double *affinity, int no_point, int no_dim, int knn, int kernel_poly_degree, int is_newton, double *Y)
{
	...
	double *newton = (double*)malloc(sizeof(double) * no_dim);
		...
		for (i = 0; i < no_point; ++i)
		{
			...
			for (k = 0; k < knn; ++k)
			{
				if (affinity[n_ink + k] > 0)
					...
					if (similarity[nn_idx[n_ink + k]] < 0)
						similarity[nn_idx[n_ink + k]] = 0;*/
			...
			if (is_newton)
				inverse_mult(Y, no_point, no_dim, gradient, newton);
			...
			for (d = 0; d < no_dim; ++d)
			{
				n_temp = n_ind + d;
				if (is_newton)
					Y[n_temp] = Y[n_temp] + (alpha * newton[d]);
				else
