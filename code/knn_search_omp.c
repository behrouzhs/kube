...
// naive openmp critical region usage - not efficient
// calculating only the upper triangular distances n(n-1)/2 and maintaining n heaps
// faster way of calculating euclidean distances: |x-y|^2 = x^2 + y^2 - 2xy
//void knn_search_omp(double *X, int no_point, int no_dim, int knn, int no_thread, int *out_index, double *out_dist)
//{
...
//
//	omp_set_num_threads(no_thread);
//#pragma omp parallel default(none) private(i, j, d, n_idx1, n_idx2, d_temp, dist) shared(X, no_point, no_dim, knn, len, heap)
//	{
//		#pragma omp for
//		for (i = 0; i < no_point; ++i)
//		{
...
//
//	omp_set_num_threads(no_thread);
//#pragma omp parallel default(none) private(i, j, ix1, ix2, n_max1, n_max2, d_temp, dist) shared(col, val, no_point, knn, start_indices, heap)
//	{
//#pragma omp for
//		for (i = 0; i < no_point; ++i)
//		{
...
//
//	omp_set_num_threads(no_thread);
//#pragma omp parallel default(none) private(i, j, ix1, ix2, n_max1, n_max2, dist) shared(col, val, no_point, knn, start_indices, heap)
//	{
//		#pragma omp for
//		for (i = 0; i < no_point; ++i)
//		{
